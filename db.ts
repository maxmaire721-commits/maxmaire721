...
import { InsertUser, users, news, contacts } from "../drizzle/schema";
import type { InsertNews, InsertContact } from "../drizzle/schema";
...
export async function getPublishedNews() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(news).where(eq(news.published, 1)).orderBy((n) => desc(n.createdAt));
}
...
export async function getContacts() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(contacts).orderBy((c) => desc(c.createdAt));
}
